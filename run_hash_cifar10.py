            ...
            for min_weight  in [   0.79, 0.81]:
                ...
                for nb_tables in NB_HASH_TABLES:
                    ...
                    for proj_dim in PROJ_DIMS:
                        print(f"dataset={DATASET}, min_weight={min_weight},hash_method = {hash_method}, kernel_method={kernel_method}, eps is ={idx}, nb_teachers={nb_teachers}, nb_tables={nb_tables}, var={np.log(var)}, proj_dim={proj_dim},  noise_scale={np.log(sigma)}, ind_budget={ind_budget},  norm={NORM}")
                 ...
                 ind_budget=ind_budget, nb_labels=10, kernel_method=kernel_method,  norm=NORM, dataset_path=DATASET_PATH)
                        print(f"accuracy={ac}")
