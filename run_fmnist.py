...
NORM = 'L2'
num_point = 12
SIGMA_KERNEL = [1.0, 1.2]
MIN_WEIGHT_LIST = [0.84, 0.86]
            ...
            for var in VARS:
                for min_weight in MIN_WEIGHT_LIST:
                    ...
                    for seed in [0, 1, 2, 3, 100]:
                        ...
                        print(f"idx is {idx}, seed is {seed}, accuracy={ac} \n")
...
test_acc_list = []
for (mul, eps) in zip(SIGMA_LIST[-2:], EPS_LIST[-2:]):
    acc = []
    for repeat in range(1):
        seed = 0
        hand_acc = hand_dp(feature=FEATURE, batch_size=256, mini_batch_size=256,
                   lr=7, optim="SGD", momentum=0.9, nesterov=False, noise_multiplier=mul,
