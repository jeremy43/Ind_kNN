            ...
            #for nb_teachers in [50, 100]:
                ...
                for nb_tables in NB_HASH_TABLES:
                    ...
                    for proj_dim in PROJ_DIMS:
                        print(f"dataset={DATASET}, min_weight={min_weight},hash_method = {hash_method}, eps is ={idx}, nb_teachers={nb_teachers}, nb_tables={nb_tables}, var={np.log(var)}, proj_dim={proj_dim},  noise_scale={np.log(sigma)}, ind_budget={np.log(ind_budget)},  norm={NORM}")
                        ac = IndividualkNN(dataset=DATASET, var=var,
                 noisy_scale=sigma, kernel_method=kernel_method,  min_weight = min_weight,  nb_teachers = nb_teachers,seed=seed,  hash_method=hash_method,  feature=FEATURE, num_query=NUM_QUERY,  num_tables=nb_tables, proj_dim=proj_dim, ind_budget=ind_budget, nb_labels=14, norm=NORM, dataset_path=DATASET_PATH)
                        print(f"accuracy={ac}")
